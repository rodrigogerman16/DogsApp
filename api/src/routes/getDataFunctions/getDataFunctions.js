...
const getApiInfo = async () => {
  ...
  const apiInfo = await apiUrl.data.map((dog) => {
    return {
      ...
      weightMax: Number.isNaN(parseInt(dog.weight.metric.split(" - ")[1]))
        ? 50
        : parseInt(dog.weight.metric.split(" - ")[1]),
      heightMin: Number.isNaN(parseInt(dog.height.metric.split(" - ")[0]))
        ? 1
        : parseInt(dog.height.metric.split(" - ")[0]),
      heightMax: Number.isNaN(parseInt(dog.height.metric.split(" - ")[1]))
        ? 10
